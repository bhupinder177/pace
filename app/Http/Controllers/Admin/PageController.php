...
class PageController extends Controller
{
    public function index(Request $request) {
        ...
        if($request->ajax()){
            ...
            $html =  view('admin.page.pageajax',['prefix'=>$this->prefix,'allData'=>$allData,'perpage'=>$perpage,'srNo'=>(request()->input('page', 1) - 1) * $perpage])->render();
            return response()->json(['html' => $html]);
        } else {
            $allData = $query->orderby('id','DESC')->paginate($perpage);
            return view('admin.page.index',['prefix'=>$this->prefix,'allData'=>$allData,'perpage'=>$perpage,'srNo'=>(request()->input('page', 1) - 1) * $perpage]);
        }
    }

    public function add() {
        $this->prefix = request()->route()->getPrefix();
        return view('admin.page.add',['prefix'=>$this->prefix]);
    }
    ...
    public function save(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if($request->hasFile('page_image')) {
                $getUniqueNo = time();
                $saveStorePath = "public/page";
                $attachmentDoc = $request->file('page_image');
            ...
            if ($request->has('id')) {
                ...
                $res = Page::where('id',$id)->update($saveData);
            } else {
                $user = new Page($saveData);
                $res =  $user->save();
            }
            ...
            if($res) {
               ...
               $response['success_message'] = 'Page Saved Successfully.';
               $response['url'] = route('page.index');
    ...
    public function edit($id) {
        $id = Crypt::decrypt($id);
        $this->prefix = request()->route()->getPrefix();
        $result = Page::whereKey($id)->first();
        return view('admin.page.edit',['result'=>$result,'prefix'=>$this->prefix]);
    }
    ...
    public function update(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if ($request->has('id')) {
                $id = Crypt::decrypt($request->id);
                $saveData['slug'] = (PageSlug::createSlug($getTitle, $id)) ?? "";
                $res = Page::where('id',$id)->update($saveData);
            } else {
                $saveData['slug'] = (PageSlug::createSlug($getTitle)) ?? "";
                $user = new Page($saveData);
            ...
            if($res) {
               $response['success']         = true;
               $response['delayTime']       = '3000';
               $response['success_message'] = 'Page Updated Successfully.';
               $response['url'] = route('page.index');
               $response['resetform'] ='true';
               return response($response);
            } else {
    ...
    public function delete(Request $request) {
        $res = Page::destroy($request['id']);
        if($res) {
            $response['success']         = true;
            $response['delayTime']       = '2000';
            $response['success_message'] = 'Page Deleted successfully.';
            return response($response);
        } else {
            $response['formErrors'] = true;
            $response['delayTime']     = '2000';
            $response['errors'] = 'Page Not Deleted.';
            return response($response);
        }
    }
}
